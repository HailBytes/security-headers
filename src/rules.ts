...
export function checkCSP(headers: RawHeaders): HeaderFinding {
  ...
  // Check for overly permissive sources (bare wildcard `*` OR scheme-only
  // sources like `https:`, `http:`, `data:`) in high-risk fetch/navigation
  // directives. Scheme-only sources are as permissive as `*` for their scheme
  // — `script-src https:` allows scripts from any HTTPS host.
  // img-src/style-src/font-src/media-src are intentionally omitted as
  // permissive sources there are low-risk and commonly legitimate.
  ...
  if (wildcarded.length > 0) {
    score -= 5;
    findings.push(`Overly permissive source (wildcard or scheme-only) in ${wildcarded.join(', ')} allows any origin`);
    recommendations.push('Replace wildcards and scheme-only sources (e.g. https:) with specific trusted domains');
  }
